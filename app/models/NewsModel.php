...
class NewsModel extends Model
{
    ...
    const SQL_GET_ONE_NEWS_BY_HREF = "SELECT * FROM news WHERE href = :href";

    const SQL_GET_NEWS_AUTHOR = "SELECT name, surname FROM profile where id = :id";

    const  SQL_SET_NEWS_VIEW = "UPDATE news SET amount_view = amount_view + 1 WHERE href = :href";
    ...
    public function getNews($href) {
        parent::$dataBase->setBasePrepare(self::SQL_SET_NEWS_VIEW, [$href]);
        return parent::$dataBase->getBasePrepare(self::SQL_GET_ONE_NEWS_BY_HREF, [$href]);
    }
